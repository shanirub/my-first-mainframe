...
void logicTask(void* params) {
    ...
    while (true) {
        ...
        if (!res.valid) {
            Serial.printf("[MCU5-LG] INVALID message: %s\n", res.detail);
        ...
        switch (type) {
            case MsgType::HEARTBEAT: {
                Serial.printf("[MCU5-LG] received HEARTBEAT from 0x%02X (#%lu)\n",
                    from, rxCount);
                ...
                if (!MessageProtocol::serialize(ack, txBuf, sizeof(txBuf))) {
                    Serial.println("[MCU5-LG] ACK serialize failed");
                    break;
                }

                BusError err = sharedBus.send(from, txBuf);
                Serial.printf("[MCU5-LG] ACK → 0x%02X: %s\n",
                    from, err == BusError::OK ? "OK" : "FAILED");
                break;
            }

            default:
                Serial.printf("[MCU5-LG] unhandled type: %d\n", type);
                break;
        }
    }
}
...
void oledTask(void* params) {
    ...
    while (true) {
        ...
        oled.showStatus("I/O CONTROLLER", line2, line3, line4);
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("[MCU5] I/O Controller starting — Phase 3");

    if (!oled.begin()) {
        Serial.println("[MCU5] OLED failed");
    }
    oled.showStatus("I/O CONTROLLER", "Starting...", "", "");

    sharedBus.init(I2C_ADDRESS);

    displayMutex = xSemaphoreCreateMutex();
    if (displayMutex == nullptr) {
        Serial.println("[MCU5] FATAL: displayMutex failed");
        return;
    }

    inboundQueue = xQueueCreate(8, sizeof(char[256]));
    if (inboundQueue == nullptr) {
        Serial.println("[MCU5] FATAL: inboundQueue failed");
        return;
    }
    ...
    Serial.println("[MCU5] Tasks created — scheduler already running");
}
