...
void logicTask(void* params) {
    ...
    while (true) {
        ...
        if (!res.valid) {
            Serial.printf("[MCU4-LG] INVALID message: %s\n", res.detail);
        ...
        switch (type) {
            case MsgType::HEARTBEAT: {
                Serial.printf("[MCU4-LG] received HEARTBEAT from 0x%02X (#%lu)\n",
                    from, rxCount);
                ...
                if (!MessageProtocol::serialize(ack, txBuf, sizeof(txBuf))) {
                    Serial.println("[MCU4-LG] ACK serialize failed");
                    break;
                }

                BusError err = sharedBus.send(from, txBuf);
                Serial.printf("[MCU4-LG] ACK → 0x%02X: %s\n",
                    from, err == BusError::OK ? "OK" : "FAILED");
                break;
            }

            default:
                Serial.printf("[MCU4-LG] unhandled type: %d\n", type);
                break;
        }
    }
}
...
void oledTask(void* params) {
    ...
    while (true) {
        ...
        oled.showStatus("JOB SCHEDULER", line2, line3, line4);
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("[MCU4] Job Scheduler starting — Phase 3");

    if (!oled.begin()) {
        Serial.println("[MCU4] OLED failed");
    }
    oled.showStatus("JOB SCHEDULER", "Starting...", "", "");

    sharedBus.init(I2C_ADDRESS);

    displayMutex = xSemaphoreCreateMutex();
    if (displayMutex == nullptr) {
        Serial.println("[MCU4] FATAL: displayMutex failed");
        return;
    }

    inboundQueue = xQueueCreate(8, sizeof(char[256]));
    if (inboundQueue == nullptr) {
        Serial.println("[MCU4] FATAL: inboundQueue failed");
        return;
    }
    ...
    Serial.println("[MCU4] Tasks created — scheduler already running");
}
